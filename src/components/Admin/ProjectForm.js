    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      >
        {loading ? (
          ...
          project ? 'Actualizar Cambios' : mode === 'reels' ? 'Guardar Reel' : mode === 'art' ? 'Guardar Arte' : mode === 'photo' ? 'Guardar Fotografía' : 'Guardar en Silo'
        )}
      </button>
    ...
    <AnimatePresence>
      {showFullClientModal && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="fixed inset-0 z-[400] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-sm"
        >
          <motion.div
